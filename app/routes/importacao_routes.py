...
@swag_from({
    ...
    'description': 'Retorna dados de importação de vinhos da Embrapa com suporte a paginação e filtros',
    'parameters': [
        {
            'name': 'page',
            'in': 'query',
...
def get_importacao():
    ...
    try:
        ...
        data = service.get_data('importacao')
...
@importacao_bp.route('/importacao/anos', methods=['GET'])
@optional_token
@swag_from({
    'tags': ['Importação'],
    'summary': 'Obter anos disponíveis',
    'description': 'Retorna lista de anos disponíveis nos dados de importação',
    'parameters': [
        {
            'name': 'Authorization',
            'in': 'header',
...
def get_importacao_anos():
    """Endpoint para obter anos disponíveis nos dados de importação"""
    try:
        service = EmbrapaService()
        data = service.get_data('importacao')
...
@importacao_bp.route('/importacao/paises', methods=['GET'])
@optional_token
@swag_from({
    'tags': ['Importação'],
    'summary': 'Obter países disponíveis',
    'description': 'Retorna lista de países disponíveis nos dados de importação',
    'parameters': [
        {
            'name': 'Authorization',
            'in': 'header',
...
def get_importacao_paises():
    """Endpoint para obter países disponíveis nos dados de importação"""
    try:
        service = EmbrapaService()
        data = service.get_data('importacao')
